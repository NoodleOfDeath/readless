...
export function Summary({
  ...
  }, [initialFormat, isRead, summary.category, summary.outlet, summary.originalDate, summary.sentiment, theme.colors.textDisabled, theme.colors.text, formatTime, openCategory, openOutlet]);
        ...
        <View row>
          ...
          <View
            ...
            {summary?.siblings && summary.siblings.length > 0 && (
              <View mh={ 12 } gap={ 6 }>
                <Text>{strings.summary.relatedNews}</Text>
                <ScrollView
                  nestedScrollEnabled
                  height={ 70 }
                  pagingEnabled>
                  <View gap={ 6 }>
                    {summary.siblings.map((sibling) => (
                      <View 
                        key={ sibling.id } 
                        gap={ 1 }
                        height={ 54 }
  ...
  }, [compact, compactMode, initialFormat, summary, shareTarget?.id, isRead, theme.colors.textDisabled, theme.colors.text, theme.colors.textDark, keywords, showShortSummary, localizedStrings.shortSummary, localizedStrings.title, translateToggle, sourceIsRead, bookmarked, playingAudio, formatTime, openSummary, onInteract, openURL, copyToClipboard, format, handlePlayAudio]);
  ...
  }, [textContent, translateToggle, format, preferredReadingFormat, handleFormatChange, theme.colors.textDark, keywords, shareTarget?.id, summary.id, summary.sentiment, summary.sentiments]);
